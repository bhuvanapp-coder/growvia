...
export function DashboardPage({ items, setPage, setSelected, reminders, toggleReminder, reminderNotice, saved, toggleSaved }) {
  const [loading, setLoading] = useState(true)
  useEffect(() => { const timer = window.setTimeout(() => setLoading(false), 420); return () => window.clearTimeout(timer) }, [])
  const deadlines = [...items].sort((first, second) => new Date(`${first.deadline}, 2026`).getTime() - new Date(`${second.deadline}, 2026`).getTime()).slice(0, 4)
  ...
  return <div className="page-wrap dashboard-page">
    ...
    {reminderNotice && <div className="reminder-notice"><Clock3 size={15} />{reminderNotice}</div>}<DeadlineRail items={deadlines} reminders={reminders} setSelected={setSelected} />
  </div>
}
...
function DeadlineRail({ items, reminders, setSelected }) { return <section className="deadline-rail"><div className="deadline-rail-heading"><div><div className="eyebrow"><Clock3 size={13} /> KEEP YOUR MOMENTUM</div><h2>Upcoming deadlines</h2></div><button className="text-button">Open calendar <ArrowUpRight size={15} /></button></div>{items.length ? <div className="deadline-grid">{items.map((item, index) => { const deadline = deadlineInfo(item.deadline); return <button disabled={deadline.isPast} className={`deadline-card ${reminders.includes(item.id) ? 'has-reminder' : ''} ${deadline.isPast ? 'past' : ''}`} key={item.id} onClick={() => setSelected(item)}><div className={`deadline-date ${index === 0 && !deadline.isPast ? 'urgent' : ''}`}><strong>{item.deadline.split(' ')[1]}</strong><span>{item.deadline.split(' ')[0]}</span></div><div className="deadline-copy"><strong>{item.title}</strong><span>{item.org} · {item.location}</span><small>{deadline.isPast ? 'Registration closed' : reminders.includes(item.id) ? '🔕 Reminder Set' : '🔔 Remind Me'}</small></div><span className="deadline-arrow"><ArrowUpRight size={16} /></span></button> })}</div> : <div className="dashboard-empty"><CalendarDays size={18} />No upcoming deadlines yet.</div>}</section> }
