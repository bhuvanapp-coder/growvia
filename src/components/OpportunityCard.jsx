...
export function OpportunityCard({ item, onOpen, reminder, onToggleReminder, saved, onToggleSaved }) {
  ...
  return <article className={`opportunity-card ${item.color}`}>
    ...
    <div className="org-name">{item.org}</div><h3>{item.title}</h3>
    ...
    <div className="meta-row"><span><CalendarDays size={14} />{item.date}</span><span><MapPin size={14} />{item.location}</span></div>
    <p className="reason"><Sparkles size={14} />{item.reason}</p>
    <div className="card-actions"><button className="text-button" onClick={() => onOpen(item)}>Why this match <ArrowUpRight size={14} /></button><button className={reminder ? 'remind-button on' : 'remind-button'} onClick={() => onToggleReminder(item.id)}><Bell size={14} />{reminder ? 'Reminded' : 'Remind me'}</button></div>
  </article>
}